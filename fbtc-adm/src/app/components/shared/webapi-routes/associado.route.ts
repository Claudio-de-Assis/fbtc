...
export class AssociadoRoute {
    ...
    getFindByFilters(nome: string, cpf: string, sexo: string, atcId: number,
         crp: string, tipoProfissao: string, tipoPublicoId: number): string {

        return AppSettings.API_ENDPOINT + this.url + `FindByFilters/${nome},${cpf},${sexo},
        ${atcId},${crp},${tipoProfissao},${tipoPublicoId}`;
    }
